...
interface AdminAuthRuntimeConfig {
  adminAuthTokenExpiresSeconds?: number | string;
  adminRefreshTokenExpiresSeconds?: number | string;
  adminAuthPrivateKey?: string;
  adminAuthPublicKey?: string;
  adminRefreshPrivateKey?: string;
  adminRefreshPublicKey?: string;
}
...
const getAdminAuthRuntimeConfig = (): AdminAuthRuntimeConfig => {
  return useRuntimeConfig() as unknown as AdminAuthRuntimeConfig;
};

export const getAuthTokenTtls = () => {
  const config = getAdminAuthRuntimeConfig();
  return {
    authTtlSeconds: clampTtl(
      config.adminAuthTokenExpiresSeconds,
      MAX_AUTH_TTL_SECONDS,
      MAX_AUTH_TTL_SECONDS,
    ),
    refreshTtlSeconds: clampTtl(
      config.adminRefreshTokenExpiresSeconds,
...
const keyToPem = (key: KeyObject, kind: "public" | "private") =>
  key
    .export({ format: "pem", type: kind === "private" ? "pkcs8" : "spki" })
    .toString();
...
const persistDevKeyPair = (kind: "access" | "refresh", pair: KeyPair) => {
  ...
  const { privatePath, publicPath } = getDevKeyFilePaths(kind);
  writeFileSync(privatePath, keyToPem(pair.privateKey, "private"), {
    encoding: "utf-8",
    mode: 0o600,
  });
  writeFileSync(publicPath, keyToPem(pair.publicKey, "public"), {
    encoding: "utf-8",
    mode: 0o644,
  });
};
...
const getAuthCrypto = (): AuthCryptoContext => {
  ...
  if (hasAccessKeys && hasRefreshKeys) {
    ...
    throw new Error(
      "Admin JWT key pairs are missing. Set ADMIN_AUTH_* and ADMIN_REFRESH_* keys.",
    );
  }
...
const verifyJwt = <T extends JwtPayloadBase>(
  ...
  if (header.alg !== "EdDSA" || header.typ !== "JWT") {
    throw createError({
      statusCode: 401,
      statusMessage: "Invalid token header",
    });
  }
...
const hashTokenId = (value: string) =>
...
export const clearAdminAuthCookies = (event: H3Event) => {
  ...
  setCookie(event, ADMIN_REFRESH_COOKIE, "", {
    ...clearOpts,
    path: "/refresh-token",
  });
...
export const getAccessTokenPayload = (
  ...
  try {
    ...
    return verifyJwt<AccessTokenPayload>(
      token,
      crypto.access.publicKey,
      "access",
      options,
    );
  } catch {
...
export const getRefreshTokenPayload = (
  ...
  try {
    const crypto = getAuthCrypto();
    return verifyJwt<RefreshTokenPayload>(
      token,
      crypto.refresh.publicKey,
      "refresh",
      options,
    );
  } catch {
...
export const getAdminUserFromAccessToken = (
  event: H3Event,
): AdminAuthUser | null => {
...
export const revokeRefreshTokenById = async (
  ...
  await AdminRefreshToken.findOneAndUpdate(
    ...
    },
    {
      revokedAt: new Date(),
      ...(replacedById
        ? { replacedByTokenIdHash: hashTokenId(replacedById) }
        : {}),
    },
  );
};
...
export const validateRefreshTokenRecord = async (

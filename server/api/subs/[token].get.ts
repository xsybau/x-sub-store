...
export default defineEventHandler(async (event) => {
  ...
  const fetchPromises = [...userUpstreams, ...globalUpstreams].map(
    async (u) => {
      try {
  ...
  userStatic.forEach((n) => {
    ...
    uris.forEach((uri) =>
      allNodes.push({ uri, priority: 3, source: "Static User" }),
    ); // Highest priority
  });
  ...
  globalStatic.forEach((n) => {
    const uris = extractNodes(n.content);
    uris.forEach((uri) =>
      allNodes.push({ uri, priority: 1, source: "Static Global" }),
    ); // Lowest priority
  });

<template>
  <div class="min-h-screen bg-gradient-to-br from-violet-50 via-white to-indigo-100 dark:from-gray-950 dark:via-gray-900 dark:to-violet-950">
    <div class="flex min-h-screen">
      ...
      >
        <div class="p-6 border-b border-default/70">
          <div class="flex items-start justify-between gap-3">
            <div class="flex items-center gap-3">
              <div class="flex size-10 shrink-0 items-center justify-center rounded-2xl bg-primary text-inverted shadow-sm">
                <UIcon name="i-heroicons-command-line" class="size-5" />
              </div>
              <div>
                <p class="text-[11px] uppercase tracking-[0.2em] text-primary/80">Control</p>
                <h1 class="text-xl font-semibold text-highlighted">V2Ray Hub</h1>
              </div>
            </div>
            <UButton
              class="lg:hidden"
        ...
        <nav class="p-4 flex-1 space-y-2 overflow-y-auto">
          ...
          >
            <div
              ...
              :class="isActive(item.to) ? 'text-primary' : 'text-muted group-hover:text-primary'"
            >
              <UIcon :name="item.icon" class="size-5" />
            </div>
            <div class="min-w-0">
              <p class="font-semibold leading-tight text-highlighted">{{ item.label }}</p>
        ...
        <div class="p-4 border-t border-default/70">
          <div class="rounded-2xl border border-default/70 bg-default/40 p-3 space-y-3">
            <UBadge color="primary" variant="subtle">Admin Console</UBadge>
            <UButton variant="soft" color="error" block @click="logout">Logout</UButton>
          </div>
        </div>
      </aside>
    ...
    <div class="fixed inset-x-0 bottom-5 z-40 flex justify-center px-4 lg:hidden">
      <UButton
        color="primary"
        variant="solid"
      ...
      >
        <UIcon :name="mobileMenuOpen ? 'i-heroicons-x-mark' : 'i-heroicons-bars-3'" class="size-4" />
